...
interface DebtorAgeingFIFOProps {
  data: LedgerEntry[];
}
...
const isSundryDebtorEntry = (entry: LedgerEntry): boolean => {
  const primary = (entry.TallyPrimary || '').toLowerCase();
  const parent = (entry.TallyParent || '').toLowerCase();
  return primary.includes('sundry debtor') || parent.includes('sundry debtor');
};
...
const DebtorAgeingFIFO: React.FC<DebtorAgeingFIFOProps> = ({ data }) => {
  ...
  const partyResults = useMemo(() => {
    ...
    const debtorEntries = data
      .filter(isSundryDebtorEntry)
    ...
    partyMap.forEach((entries, party) => {
      ...
      const applyCredit = (creditAmount: number) => {
        let remaining = creditAmount;
        for (let i = 0; i < receivableQueue.length && remaining > 0; i++) {
          const item = receivableQueue[i];
          if (item.outstandingAmount <= 0) continue;
          const used = Math.min(item.outstandingAmount, remaining);
          item.outstandingAmount -= used;
          remaining -= used;
        }
        if (remaining > 0) carryCredit += remaining;
      };

      const addDebitInvoice = (voucherNumber: string, invoiceDate: string, amount: number) => {
        let pending = amount;
        if (carryCredit > 0) {
          const offset = Math.min(carryCredit, pending);
          carryCredit -= offset;
          pending -= offset;
        }
        if (pending > 0) {
          receivableQueue.push({
            voucherNumber,
            invoiceDate,
      ...
      if (openingBalance < 0) addDebitInvoice('Opening Balance', firstDate, Math.abs(openingBalance));
      else if (openingBalance > 0) applyCredit(openingBalance);

      sorted.forEach((entry) => {
        const amount = toNumber(entry.amount);
        const invoiceDate = entry.date || firstDate;
        const voucherNumber = entry.invoice_number || entry.voucher_number || 'UNKNOWN';
        if (amount < 0) addDebitInvoice(voucherNumber, invoiceDate, Math.abs(amount));
        if (amount > 0) applyCredit(amount);
      });

      const invoices: OutstandingInvoice[] = receivableQueue
      ...
      const closingReceivable = invoices.reduce((acc, inv) => acc + inv.outstandingAmount, 0);
      results.push({
        party,
        openingBalance,
        closingBalance,
        closingReceivable,
        advanceAmount: carryCredit,
        invoices,
        bucketTotals,
      });
    });
    ...
    return results.sort(
      (a, b) => b.closingReceivable - b.advanceAmount - (a.closingReceivable - a.advanceAmount)
    );
  }, [data, asOfDateText]);
  ...
  const totals = useMemo(() => {
    return filteredPartyResults.reduce(
      (acc, party) => {
        ...
        acc.totalReceivable += party.closingReceivable;
  ...
  const reconciliation = useMemo(() => {
    const ledgerClosing = partyResults.reduce(
      (acc, party) => {
        ...
        acc.fifoReceivable += party.closingReceivable;
    ...
    const diffDr = ledgerClosing.fifoReceivable - ledgerClosing.ledgerClosingDr;
    const diffCr = ledgerClosing.fifoAdvance - ledgerClosing.ledgerClosingCr;
    const combinedFifo = ledgerClosing.fifoReceivable + ledgerClosing.fifoAdvance;
    ...
    const signedFifo = ledgerClosing.fifoAdvance - ledgerClosing.fifoReceivable;
  ...
  const exportExcel = async () => {
    ...
    const summaryRows = filteredPartyResults.map((party) => {
      ...
      return {
        ...
        'FIFO Closing Receivable': party.closingReceivable,
        'FIFO Advance': party.advanceAmount,
        'Net FIFO (Dr-Cr)': party.closingReceivable - party.advanceAmount,
    ...
    summaryAoA.push(['Debtor FIFO Ageing Summary - Sundry Debtors']);
    summaryAoA.push([`As On: ${asOn}`]);
    summaryAoA.push(['Source: Ledgers under Sundry Debtors (TallyPrimary/TallyParent).']);
    summaryAoA.push([]);
    ...
    summaryAoA.push([
      ...
      totals.totalAdvance,
      totals.totalReceivable - totals.totalAdvance,
      totals.invoiceCount,
    ...
    detailsAoA.push(['Debtor FIFO Ageing Details']);
    detailsAoA.push([`As On: ${asOn}`]);
    detailsAoA.push(['Source: Sundry Debtors only']);
    detailsAoA.push([]);
    detailsAoA.push([
      'Party',
      'Voucher / Invoice',
    ...
    wsRecon['!autofilter'] = { ref: `A4:E${reconAoA.length}` };
      ...
      XLSX.writeFile(wb, `Debtor_Ageing_FIFO_${stamp}.xlsx`, { compression: true });
    } catch (error) {
      console.error(error);
      window.alert('Unable to export Debtor Ageing Excel. Please retry.');
    }
  };
...
export default DebtorAgeingFIFO;
